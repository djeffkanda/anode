...
class KitNet(nn.Module):
    def __init__(self, enc_layers: list = None, dec_layers: list = None, n_aes=5, **kwargs):
        latent_dim = kwargs.get("ae_latent_dim", 1)
        cond = (enc_layers and dec_layers) or (kwargs.get("dataset_name", None) and kwargs.get("in_features", None))
        assert cond, "please provide either the name of the dataset and the number of features or specify the encoder " \
                     "and decoder layers "
        ...
        # Building ensemble AEs
        in_features = kwargs.get("in_features")
        self.in_features = in_features

        enc_layers = [
            (in_features, int(in_features * .2), nn.ReLU()),
            (int(in_features * .2), int(in_features * .2 * .75), nn.ReLU()),
            (int(in_features * .2 * .75), int(in_features * .2 * .5), nn.ReLU()),
        ]
        dec_layers = [
            (int(in_features * .2 * .5), int(in_features * .2 * .75), nn.ReLU()),
            (int(in_features * .2 * .75), int(in_features * .2), nn.ReLU()),
            (int(in_features * .2), in_features, None)  # nn.Sigmoid()),
        ]

        # self.ae_test = AutoEncoder(enc_layers, dec_layers, **kwargs)
        self.hstack_ae = nn.ModuleList([AutoEncoder(enc_layers, dec_layers, **kwargs) for _ in range(n_aes)])

        # Building the output AE
        enc_layers = [
            (n_aes, 4, nn.ReLU()),
            (4, latent_dim, nn.ReLU()),
        ]
        dec_layers = [
            (latent_dim, 4, nn.ReLU()),
            (4, n_aes, None)
        ]

        self.output_ae = AutoEncoder(enc_layers, dec_layers, **kwargs)
        # Randomly initialize the model center and make it learnable
        self.latent_center = nn.Parameter(torch.randn(1, latent_dim))  # latent_dim))
        # self.latent_center = nn.Parameter(torch.randn(1, int(in_features * .2 * .5)))  # latent_dim))
        self.n_aes = n_aes

    def rmsencode(self, x):
        mse = nn.MSELoss()

        hstack_out = torch.empty(self.n_aes, x.shape[0]).to(x)
        for i in range(self.n_aes):
            _, x_hat = self.hstack_ae[i](x)
            # vmse = mse(x, self.hstack_ae[i](x))
            hstack_out[i] = ((x - x_hat) ** 2).sum(-1)  # torch.sqrt(((x - x_hat)**2).sum(-1))
            # hstack_out[i] = torch.sqrt(hstack_out[i])
        # z = F.normalize(z, p=2, dim=-1)
        hstack_out = torch.transpose(hstack_out, 0, 1)
    ...
    def forward(self, x):
        ...
        hstack_out = self.rmsencode(x)

        # _, hstack_out = self.hstack_ae[0](x)

        # z, output = self.hstack_ae[1](hstack_out)

        z, output = self.output_ae(hstack_out.detach())

        # z, output = self.hstack_ae[1](x)#self.ae_test(x)
        return hstack_out, z, output
