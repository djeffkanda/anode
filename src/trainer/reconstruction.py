...
class AutoEncoderTrainer(BaseTrainer):
    ...
    def train_iter(self, X, **kwargs):
        ...
        reg_n = self.kwargs.get('reg_n', 0)  # 1e-3
...
class KitNetTrainer(AutoEncoderTrainer):
    # optim.Adam(self.model.parameters(), lr=self.lr, weight_decay=weight_decay)
    ...
    def __init__(self, model,
        ...
        super(AutoEncoderTrainer, self).__init__(model, batch_size, lr, n_epochs, n_jobs_dataloader, device, **kwargs)
        # pms = list(self.model.parameters())
        # self.optimizer = torch.optim.SGD(self.model.parameters(), lr=self.lr, momentum=.9)

    def score(self, sample: torch.Tensor):
        X, code, X_prime = self.model(sample)
        return ((X - X_prime) ** 2).sum(axis=1)

    def train_iter(self, S, **kwargs):
        # X, code, X_prime = self.model(S)
        ensemble_out, code, X_prime = self.model(S)
        X = ensemble_out.detach()
        # code, X_prime = self.model(X)
        reg_n = self.kwargs.get('reg_n', 0)  # 1e-3
        reg_a = self.kwargs.get('reg_a', 0)  # 1e-1 * 0
        alpha = kwargs['contamination_rate']
        ...
        ens_losses = ensemble_out.sum(axis=-1).mean()
        return loss + ens_losses  # torch.sqrt(loss)
